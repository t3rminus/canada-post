...
class CanadaPostClient {
	constructor(userId, password, customer, lang, useTestEndpoint) {
		this.endpoint = !useTestEndpoint && process.env.NODE_ENV === 'production' ? CanadaPostClient.ENDPOINT : CanadaPostClient.ENDPOINT_DEV;
	...
	async getShipments(from, to) {
		...
		const result = await this._request(`ncshipment`, params, 'application/vnd.cpc.ncshipment-v4+xml');

		CanadaPostClient.checkResultFormat(result, 'non-contract-shipments.link');
